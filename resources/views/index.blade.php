          ...
          <legend style="color: green; font-weight: bold;">LARAVEL 7.X CRUD EXAMPLE - CODECHIEF
           ...
           <tbody>
              @forelse ($customers as $customer)
                ...
                <td class="text-center">{{ $customer->qualification }}</td>
                <td class="text-center">
                  <a href="{{ route('edit',$customer->slug) }}" class="btn btn-sm btn-outline-danger py-0">Edit</a>
                  <a href="{{ route('view',$customer->slug) }}" class="btn btn-sm btn-outline-danger py-0">View</a>
                  ...
                  <form id="delete-{{$customer->slug}}" method="post" action="{{route('delete',$customer->slug)}}" style="display: none;">
                  @csrf
                  @method('DELETE')
                </form>
                </td>
